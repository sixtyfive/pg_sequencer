...
This version have changes to solve issue with rails's `schema.rb`.

The issue is `create_table` creates sequence for primary key.

pg_sequencer dumps all sequences, so, `rake db:schema:load` not works now
because we apply `create_table` sequences twice:
* when table creates
* when load sequence dumped by `pg_sequencer`

The temparary solutions is exclude all sequences ends with `_id_seq`
