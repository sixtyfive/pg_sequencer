...
The methods closely mimic the manipulation syntax of the PostgreSQL SQL for CREATE SEQUENCE, DROP SEQUENCE and
ALTER SEQUENCE. The existence check scans pg_class, and fetching the next value uses NEXTVAL. See the REFERENCES section below for more information.
...
If it doesn't already exist, create a sequence called "seq_user", incrementing by 1, min of 1, max of 2000000, starts at 1, caches 10 values, and disallows cycles:
...
=== Only remove a sequence if it exists and has a value > 1000

  unless sequence_exists?("seq_products") && sequence_value("seq_products") > 1000
    drop_sequence "seq_products"
  end
